/*
	...
	Filename:     LZ773.C
	Decompressr:  lz773d.c
	Description:  **** Basic LZ77/LZSS (len,position) pair output ****
	Author:       Gerald Tamayo
	Date:         August 15, 2004

	The program here only serves as a working example of LZ77/LZSS.

	Improving the search function is very important for
	LZ77/LZSS; you can use a better search function like a hash
	...
	ENCODING:
		...
		A match length less than MIN_MATCH_LEN gets a length of 1,
		and is independently coded with a prefix bit (LZSS).

		The match lengths (MIN_MATCH_LEN..PAT_BUFSIZE) are thus mapped
		to codes (0..PAT_BUFSIZE-MIN_MATCH_LEN).
...
dpos_t search( uchar w[], uchar p[] )
{
	...
	i = lzlist[ (uchar) p[pat_cnt] ]; /* start window search here.. */
...
void put_codes( dpos_t *dpos )
{
	...
	if ( dpos->len < MIN_MATCH_LEN ) {
		dpos->len = 1;
		put_ONE();     /* LZSS prefix bit. */
	}
	else {
		put_ZERO();    /* LZSS prefix bit. */
		k = dpos->len;
		k -= MIN_MATCH_LEN;
		/* output length code. */
