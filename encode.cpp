...
void encode();
bool first(int);
string retCodeData(string);
string givenCode(char);
...
void createNode(string str, int num, int freq, int p, string c) {
    Node temp;
    temp.ch = str;
    temp.no = num;
    temp.weight = freq;
...
// Returns true if symbol is first appearance
bool first(int n) {
    for (int a = 0; a < n; a++) {
        if (data[a] == data[n]) {
            return false;
...
// Returns code for the data element
string retCodeData(string str) {
...
// Returns code for specific data element
string givenCode(char str) {
    switch (str) {
    case 'a':
        return "00000";
        break;
...
// Returns the node index with max node no. in block else -1
